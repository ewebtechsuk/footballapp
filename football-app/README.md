...
2. **Launch the CI login flow** – run `firebase login:ci`. The command opens a browser so you can sign in with a Google account
   ...
   terminal. Copy this value immediately; it will not be shown again. The login flow is fully interactive, so it cannot
   complete inside non-interactive shells or automated test runs—you must finish the OAuth prompt in a browser (or use the
   out-of-band device flow described below). Recent versions of the CLI print a warning that `firebase login:ci` is deprecated
   in favour of service account credentials—tokens generated today continue to work, but you should plan to migrate to the
   `GOOGLE_APPLICATION_CREDENTIALS` workflow described below.
